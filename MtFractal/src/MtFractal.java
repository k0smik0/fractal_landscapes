 ...
 */
//package net.iubris.mtfractal.applet;
...
public class MtFractal extends Applet implements Runnable {
	...
	void createStartRectangles() {
		...
		_width = b.width - 2 * _margin;
		_height = b.height - 2 * _margin - _corner_offset;
...
----- */
	...
	FRectangleList subdivide() {
		...
		Enumeration e = _rectangles.elements();
		FRectangle r;
...
//		_h = _h + (int)(factor * ((xdiff + ydiff) / MtFractal._H));
		...
		double pow = Math.pow(0.5, (factor) * MtFractal._H/ 2);
...
class FRectangleListEnumeration implements Enumeration {
