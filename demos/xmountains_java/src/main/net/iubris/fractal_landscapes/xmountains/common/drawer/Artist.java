package net.iubris.fractal_landscapes.xmountains.common.drawer;
...
public class Artist{
	...
	float ambient = 0.3F; /* level of ambient light */
	double contrast = 1.0; /*
									 * contrast,
	...
	double altitude = 2.5;
	double distance = 4.0;
	double phi = (40.0 * Math.PI) / 180.0;/* angle of the light (vertical plane) */
	double alpha = 0.0; /*
								 * angle of the light (horizontal plane)
								 * must have -pi/4 < alpha < pi/4
								 */
	double base_shift = 0.5; /* offset from calcalt to artist coordinates */
	double sealevel = 0.0;
	double stretch = 0.6; /* vertical stretch */
	boolean draw_map = false;
	boolean reflec = true;
	int repeat = 20;
	int pos = 0;
	public int scroll = 0;
...
/* {{{ set_clut*/
	...
	private void set_clut() {
		...
		if (debug)
			System.out.println("set_clut called");
	...
	public synchronized void init_artist_variables() {
		...
		mountains.init(); // We have to initialize to ensure width etc are set.
		...
		vscale = stretch * (double) mountains.width / mountains.fwidth;
		...
		varience = Math.pow(mountains.mwidth, (2.0 * mountains.fdim));
	...
	private int get_col(double p, double p_minus_x, double p_minus_y, double shadow) {
		...
		effective = (p + (varience * contour * (1.0 / (1.0 + hypot_sqr))));
		/* }}} */
		...
		if (p >= shadow) {
			...
			dshade += ((double) lstrength * ((delta_x * x_fact) + (delta_y * y_fact) + sin_phi));
		}
	...
	synchronized int[] makemap() {
		...
		if (debug)
			System.out.println("makemap called");
		...
		AtomicReference<Integer> iar = new AtomicReference<>();
		CountDownLatch latch = new CountDownLatch(1);
		Runnable runnable = new Runnable() {
			@Override
			public void run() {
				if (iar.get()<width) {
					int i = iar.getAndUpdate(new UnaryOperator<Integer>() {
						@Override
	...
	private void blank_region(int lx, int ly, int hx, int hy) {
		if (debug)
			System.out.println("blank_region called");
		graphics.setColor(clut[SKY]);
		graphics.fillRect(lx, ly, hx - lx + 1, hy - ly + 1);
	}
	...
	private void scroll_screen(int dist) {
		if (debug)
			System.out.println("scroll_screen called");
		if (dist > 0) {
			graphics.copyArea(dist, 0, graph_width - dist, graph_height, -dist, 0);
			blank_region(graph_width - dist, 0, graph_width, graph_height);
		} else {
			graphics.copyArea(0, 0, graph_width - dist, graph_height, -dist, 0);
...
//		System.out.println("plot_column!");
		int l[];
		int j;
		int mapwid;
		...
		if (debug) {
			System.out.println("plot_column called");
		}
		...
		if (draw_map) {
			...
			/*ExecutorService cachedThreadPool = Executors.newFixedThreadPool(16);
			int maximum = graph_height - mapwid;
			System.out.println(maximum);
			AtomicReference<Integer> arJ = new AtomicReference<>();
			cachedThreadPool.execute(()->{
				Integer jj = arJ.getAndUpdate(new UnaryOperator<Integer>() {
					@Override
					public Integer apply(Integer arg0) {
						return arg0+1;
					}
				...
				if (jj<maximum) {
					plot_pixel(pos, ((graph_height - 1) - jj), BLACK);
				}
			});
			...
			/*ExecutorService cachedThreadPool2 = Executors.newFixedThreadPool(16);
			int maximum2 = mapwid;
			System.out.println(maximum2);
			AtomicReference<Integer> arJ2 = new AtomicReference<>();
			cachedThreadPool.execute(()->{
				Integer jj = arJ2.getAndUpdate(new UnaryOperator<Integer>() {
					@Override
					public Integer apply(Integer arg0) {
						return arg0+1;
					}
				...
				if (jj<maximum2) {
//					plot_pixel(pos, ((graph_height - 1) - jj), BLACK);
					plot_pixel(pos, ((mapwid - 1) - jj), l[jj]);
				}
			});
